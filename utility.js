...
for (i=0;i<jsonContents.assets.length;i++)
{
    ...
    // console.log(i + " : " + jsonContent.name)
    // console.log("\n");    
}

fs.writeFile('assetnames.json', JSON.stringify(output), function (err) {
    if (err) 
        return console.log(err);
    console.log('assetnames written to file: assetnames.json');
});

fs.writeFile('entrypoints.json', JSON.stringify(jsonContents.entrypoints), function (err) {
    if (err) 
        return console.log(err);
    console.log('entrypoints written to file: entrypoints.json');
});
